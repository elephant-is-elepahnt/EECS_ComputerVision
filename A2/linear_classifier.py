...
def svm_loss_vectorized(W, X, y, reg):
  ...
  margin = scores - correct_class_score.view(X.shape[0], -1)+1
  maxmargin = torch.maximum(torch.zeros_like(margin), margin)
  maxmargin[range(X.shape[0]), y] = 0
  loss = torch.mean(maxmargin.sum(axis = 1))
  loss += reg * torch.sum(W * W)
  ...
  dL = maxmargin
  dL[maxmargin>0] = 1
  row_sum = dL.sum(axis = 1, keepdims = True)
  dL[range(X.shape[0]), y] = -row_sum.t()
...
def sample_batch(X, y, num_train, batch_size):
  ...
  #########################################################################
  # Replace "pass" statement with your code
  sample = torch.randint(0, X.shape[0], (batch_size, ))
  X_batch = X[sample, :]
  y_batch = y[sample]
  #########################################################################
  #                       END OF YOUR CODE                                #
...
def svm_get_search_params():
  ...
  regularization_strengths = [1e0, 5, 1e1, 20]
  ###########################################################################
  #                           END OF YOUR CODE                              #
  ###########################################################################
...
def test_one_param_set(cls, data_dict, lr, reg, num_iters=2000):
  ...
  # num_iters = 100
# X_train, y_train, learning_rate=1e-3, reg=1e-5, num_iters=100,
            # batch_size=200, verbose=False
  # Replace "pass" statement with your code
...
def softmax_loss_naive(W, X, y, reg):
  ...
  num_train = X.shape[0]
  loss = 0.0
  y_one_hot = torch.zeros(X.shape[0], W.shape[1]).cuda()
  y_one_hot[range(X.shape[0]), y] = 1
  for i in range(num_train):
    scores = W.t().mv(X[i])
    C = torch.max(scores)
    scores -= (C)
    ...
    row_sum = scores.sum()
    temp = scores/row_sum
    loss += -torch.log(temp[y[i]])

    d_softmax = (temp-y_one_hot[i])
    dW += torch.matmul(X[i].view(-1, 1), d_softmax.view(1, -1))
...
def softmax_loss_vectorized(W, X, y, reg):
  ...
  scores = torch.mm(X, W)
  max_num, _ = torch.max(scores, axis = 1, keepdims = True)
  scores += -max_num
  #exponential
  scores = torch.exp(scores)
  #row_sum
  ...
  ans_scores = scores * y_one_hot
  loss -= torch.mean(torch.log(ans_scores.sum(axis = 1, keepdims = True)))
  
  loss += reg * torch.sum(W * W)

  temp = scores - y_one_hot
...
def softmax_get_search_params():
  ...
  # Replace "pass" statement with your code
  learning_rates = [1e-3, 1e-2, 1e-1]
  regularization_strengths = [1e-1, 1e-2, 1e0, 1e1]
  ###########################################################################
  #                           END OF YOUR CODE                              #
  ###########################################################################
