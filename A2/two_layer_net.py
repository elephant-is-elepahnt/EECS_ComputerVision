...
def nn_forward_backward(params, X, y=None, reg=0.0):
    ...
    y_one_hot[range(scores.shape[0]), y] = 1

    C, _ = torch.max(scores, axis = 1, keepdims = True)
    scores -= C
    scores = torch.exp(scores)
    row_sum = scores.sum(axis = 1, keepdims = True)
    ans_scores = scores/row_sum

    loss = -torch.mean(torch.log(torch.sum(ans_scores*y_one_hot, axis = 1)))
    ...
    ###########################################################################
    # Replace "pass" statement with your code
    temp = ans_scores-y_one_hot
    dW2 = torch.mm(h1.t(), temp)
    dW2 /= N
    dW2 += reg * 2 * W2
    grads['W2'] = dW2

    dX2 = torch.mm(temp, W2.t())
    temp = torch.mm(X, W1)
    dX2[temp<0] = 0
    dW1 = torch.mm(X.t(), dX2)
    dW1 /= N
    dW1 += reg * 2 * W1
    grads['W1'] = dW1
    ###########################################################################
...
def nn_get_search_params():
  ...
  learning_rate_decays = [1.0, 0.95, 0.85]
  ###########################################################################
  #                           END OF YOUR CODE                              #
  ###########################################################################
    ...
    for hd in hidden_sizes:
      for rs in regularization_strengths:
        for lrd in learning_rate_decays:
          i += 1
          print('Training Softmax %d / %d with learning_rate=%e, reg=%e, lrd=%e, hidden=%d'
                    % (i, num_models, lr, rs, lrd, hd))
