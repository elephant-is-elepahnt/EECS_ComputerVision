...
class VAE(nn.Module):
    ...
    def forward(self, x):
        ...
        #(1)
        N, C, H, W = x.size()
        after_encoder = self.encoder(x)
        #mu, logvar이 0,1이 되도록 훈련시키는거
        mu = self.mu_layer(after_encoder)
        logvar = self.logvar_layer(after_encoder)
        #여기서 샘플링하는거
...
def reparametrize(mu, logvar):
    ...
    epsilon = torch.randn((N,Z), dtype = mu.dtype, device = mu.device)
    #this makes z that has mean as mu and var as var
    z = torch.sqrt(var)*epsilon + mu
    ################################################################################################
    #                              END OF YOUR CODE                                                #
    ################################################################################################
    return z
...
def loss_function(x_hat, x, mu, logvar):
    ...
    #D_KL은 모든 배치들의 값을 다 더함 
    #BCE는 기본은 모든 배치들의 값을 평균내는것 -> 이것도 더해줘야된다. 그래서 reduction 건들임
    BCE = nn.functional.binary_cross_entropy(x_hat, x, reduction = 'sum')
    var = torch.exp(logvar)
    D_KL = -0.5 * torch.sum(1+logvar-mu**2-var)

    loss = (BCE + D_KL)/x.size()[0]
    ################################################################################################
    #                            END OF YOUR CODE                                                  #
    ################################################################################################
    return loss
