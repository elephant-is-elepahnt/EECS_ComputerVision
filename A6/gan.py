...
def sample_noise(batch_size, noise_dim, dtype=torch.float, device='cpu'):
  ...
  noise = -1 + torch.rand((batch_size, noise_dim), dtype = dtype, device = device)*2
...
def discriminator_loss(logits_real, logits_fake):
  ...
  # Replace "pass" statement with your code
  N = logits_real.size()
  true_label = torch.ones(N, device = logits_real.device, dtype = logits_real.dtype)
  false_label = torch.zeros(N, device = logits_real.device, dtype = logits_real.dtype)

  a = F.binary_cross_entropy_with_logits(logits_real, true_label)
  b = F.binary_cross_entropy_with_logits(logits_fake, false_label)
...
def generator_loss(logits_fake):
  ...
  false_label = torch.ones(N, device = logits_fake.device, dtype = logits_fake.dtype)
  loss =  F.binary_cross_entropy_with_logits(logits_fake, false_label)
