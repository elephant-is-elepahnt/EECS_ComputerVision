...
def compute_distances_one_loop(x_train, x_test):
  ...
  for i in range(num_train):
      temp = torch.sqrt(torch.square(x_train[i]-x_test).sum(axis = 1))
      dists[i,:] = temp
...
def compute_distances_no_loops(x_train, x_test):
  ...
  x_train_sq = torch.square(x_train).sum(axis = 1, keepdims = True)
  x_test_sq = torch.square(x_test).sum(axis = 1) 
  
  temp = x_train_sq + x_test_sq
  
  multi = 2*torch.mm(x_train, torch.transpose(x_test, 1, 0))
  
  dists = torch.sqrt(temp-multi)
  ##############################################################################
  #                             END OF YOUR CODE                               #
  ##############################################################################
  return dists
...
def predict_labels(dists, y_train, k=1):
  ...
  for i in range(num_test):
      target = dists[:,i]
      value, indices = torch.topk(target, k = k,largest = False)
      y = [y_train[i] for i in indices]
      pick = y[0]
      for i in y:
          if y.count(i) > y.count(pick):
              pick = i
          elif y.count(i) == y.count(pick) : 
              if i < pick:
                  pick = i
      y_test.append(pick)
  ...
  y_pred = torch.Tensor(y_test).type(torch.int64)
...
def knn_cross_validate(x_train, y_train, num_folds=5, k_choices=None):
  ...
  ##############################################################################
  # Replace "pass" statement with your code
  for _, value in enumerate(torch.chunk(x_train, num_folds)):
      x_train_folds.append(value)
  
  for _, value in enumerate(torch.chunk(y_train, num_folds)):
      y_train_folds.append(value)
  ...
  for k in k_choices:
    ...
    for i in range(num_folds):
        ...
        for j, value in enumerate(x_train_folds):
            if i != j:
                x_fold = torch.cat((x_fold, value))
                y_fold = torch.cat((y_fold, y_train_folds[j]))
            else:
                x_val= torch.cat((x_val, value))
                y_val = torch.cat((y_val, y_train_folds[j]))
                
           
        model = KnnClassifier(x_fold, y_fold)
        acc.append(model.check_accuracy(x_val, y_val, k = k))
