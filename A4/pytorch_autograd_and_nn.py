...
class ThreeLayerConvNet(nn.Module):
  ...
  #   1. Convolutional layer with channel_1 5x5 filters with zero-padding of 2  
    ...
    ############################################################################
    # Replace "pass" statement with your code
    self.conv1 = torch.nn.Conv2d(in_channel, channel_1, kernel_size = 5, padding = 2)
    self.relu = torch.nn.ReLU()
    self.conv2 = torch.nn.Conv2d(channel_1, channel_2, kernel_size = 3, padding = 1)
    self.fc = torch.nn.Linear(channel_2 * 32 * 32, num_classes)
...
class PlainBlock(nn.Module):
  def __init__(self, Cin, Cout, downsample=False):
    ...
    if downsample == True:
      layers.append(nn.Conv2d(Cin, Cout, kernel_size = 3, stride = 2, padding = 1))
    else:
      layers.append(nn.Conv2d(Cin, Cout, kernel_size = 3, stride = 1, padding = 1))

    layers.append(nn.BatchNorm2d(num_features = Cout))
    layers.append(nn.ReLU())
    layers.append(nn.Conv2d(Cout, Cout, kernel_size = 3, padding = 1))
  ...
  def __init__(self, Cin, Cout, downsample=False):
    ...
    if downsample == False:
      if Cin == Cout:
        ...
        self.shortcut = nn.Sequential(nn.Conv2d(Cin, Cout, kernel_size = 1))
    else : 
      self.shortcut = nn.Sequential(nn.Conv2d(Cin, Cout, kernel_size = 1, stride = 2))
...
class ResNet(nn.Module):
  def __init__(self, stage_args, Cin=3, block=ResidualBlock, num_classes=10):
    ...
    for i in range(len(stage_args)):
      layers.append(ResNetStage(stage_args[i][0], stage_args[i][1], stage_args[i][2], stage_args[i][3], block))
  ...
  def forward(self, x):
    ...
    ############################################################################
    # Replace "pass" statement with your code
    scores = self.module(x)
    #Global Average를 하는법!
    avg = nn.AvgPool2d(scores.size()[2:])
    scores = avg(scores).view(scores.size()[0], -1)
