...
def content_loss(content_weight, content_current, content_original):
    ...
    F = content_current.view(1, C_l, -1)
    P = content_original.view(1, C_l, -1)
...
def gram_matrix(features, normalize=True):
    ...
    gram = torch.zeros((N, C, C), device = features.device, dtype = features.dtype)
...
def style_loss(feats, style_layers, style_targets, style_weights):
    ...
    # Replace "pass" statement with your code
    E = 0
    for l in range(len(style_targets)):
        E += style_weights[l]*torch.sum((gram_matrix(feats[style_layers[l]])-style_targets[l])**2)
    ##############################################################################
    #                               END OF YOUR CODE                             #
    ##############################################################################
    return E
...
def tv_loss(img, tv_weight):
    ...
    img_i = img[:,:, :-1, 1:]
    img_j = img[:,:, 1:, :-1]
    loss = tv_weight * (torch.sum((img_org-img_i)**2) + torch.sum((img_org-img_j)**2))
    ##############################################################################
    #                               END OF YOUR CODE                             #
    ##############################################################################
    return loss